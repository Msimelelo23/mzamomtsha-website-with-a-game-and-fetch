...
var message ;
function openWindow() {
  message = window.open("message.html","message","height=100, width=480, top=300,left=430");
...
// Need to change function validate()
function validate() {
...
if(forFirstName(firstName))
{
if (forLastName(lastName))
{
...
function forFirstName(firstName) { 
    var fNameRGEX = /^[A-Za-z]+$/;

    if(firstName.value.match(fNameRGEX)) {
        return true;
    } else {
        alert('Fisrt Name must have alphabet characters only');
        firstName.focus();
        return false;
    }
}
function forLastName(lastName) { 
    var lNameRGEX = /^[A-Za-z]+$/;

    if(lastName.value.match(lNameRGEX)) {
        return true;
    } else {
...
function gradeSelect(grade) {
    ...
    if(grade.value == "Default") {
        alert('Select a grade from the list');
        grade.focus();
        return false;
    }
    else {
...
function ValidateEmail(uemail) {
    var mailformatRGEX = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    
    if(uemail.value.match(mailformatRGEX)) {
        return true;
    }
    else {
